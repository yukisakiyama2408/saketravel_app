        ...
        {region && (
          <div className="pb-8">
            ...
            <div className="flex items-start justify-between px-5 pt-2 pb-4">
              <div>
                <h2 className="text-xl font-bold text-[#0D1B2A]">{region.name}</h2>
                <p className="text-sm text-[#0D1B2A]/60">{region.country}</p>
              </div>
              <button
                onClick={onClose}
              ...
              {activeTab === "drinks" && (
                <ul className="space-y-3">
                  {drinks.length === 0 ? (
                    ...
                    drinks.map((d) => (
                      <li key={d.id} className="border border-[#0D1B2A]/10 rounded-lg p-3 bg-white">
